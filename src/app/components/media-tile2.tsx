  ...
  return (
    <div data-cid={cids[0]} className="h-6 flex relative flex-col justify-start items-center">
      <div data-cid={cids[1]} className={cn("block", styles.className)}>
        <img data-cid={cids[2]} className={cn("w-full inline-block max-w-full overflow-clip object-contain align-middle", styles.className2)} data-component={d.kind} alt="" src={d.imgSrc} />
      </div>
      ...
      <div data-cid={cids[5]} className={cn("h-full block absolute top-0 min-w-0", styles.className4)}>
        <a data-cid={cids[6]} className={cn("block absolute top-0 z-4 max-w-full cursor-pointer", styles.className5)} data-component={d.kind2} aria-label={d.ariaLabel} href={d.href} target={d.target} />
      </div>
    </div>
  );
}
